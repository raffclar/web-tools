  ...
  `;
    ...
    switch (version) {
      case 1:
        ...
        <p><strong>Description:</strong> Generated from a combination of timestamp, clock sequence, and a node identifier.</p>
        <p><strong>Use Cases:</strong> Useful when you need to sort UUIDs by time or ensure uniqueness across different machines without a central authority.</p>
        <p><strong>Drawbacks:</strong> Potential privacy concerns as it can reveal the exact time of generation and, in some implementations, the MAC address. However, many modern libraries mitigate this by using a random 48-bit value or by setting the multicast bit instead to indicate it is not a MAC address.</p>
        <ul>
          ...
          <li><strong>Node Identifier:</strong> ${node}</li>
        </ul>
      `;
        break;
      case 3:
      case 5:
        const hashAlg = version === 3 ? "MD5" : "SHA-1";
        analysis += `
        <h3>Version ${version} (Name-based):</h3>
        <p><strong>Description:</strong> Generated by hashing a namespace identifier and a name using the ${hashAlg} algorithm.</p>
        <p><strong>Use Cases:</strong> Ideal when you need a deterministic UUID that will always be the same for the same input name and namespace. Often used for linking records across systems.</p>
        <p><strong>Drawbacks:</strong> Deterministic nature means it doesn't provide randomness. Version 3 uses MD5 which is cryptographically broken; Version 5 (SHA-1) is preferred but also not considered cryptographically secure for all purposes.</p>
      `;
        break;
      case 4:
        analysis += `
        <h3>Version 4 (Random):</h3>
        <p><strong>Description:</strong> Generated using cryptographically secure random (CSPRNG-based) or pseudo-random numbers.</p>
        <p><strong>Use Cases:</strong> The most common type of UUID. Excellent for general purpose unique identifiers where no relationship between UUIDs is needed.</p>
        <p><strong>Drawbacks:</strong> Small (though statistically negligible) risk of collision. Not naturally sortable by creation time.</p>
      `;
        break;
      case 7:
        analysis += `
        <h3>Version 7 (Time-ordered):</h3>
        <p><strong>Description:</strong> A standard that combines a Unix timestamp with random data.</p>
        <p><strong>Use Cases:</strong> Excellent for database primary keys as they are naturally sortable by time, improving indexing performance while maintaining high entropy.</p>
        <p><strong>Drawbacks:</strong> Standardized in RFC 9562 (2024), so older systems or libraries might not fully support or recognize it yet.</p>
      `;
        break;
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("generateNameBased").addEventListener("click", () => {
    ...
    // Hide name-based inputs after generation
    document.getElementById("namespaceInput").style.display = "none";
  });
...
function generateAndDisplayUUID(version) {
  try {
    ...
    // Hide name-based inputs when generating other types
    document.getElementById("namespaceInput").style.display = "none";
